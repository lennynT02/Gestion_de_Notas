...
def agregar():
  ...
  with open("calificaciones.txt", "w") as calificaciones:
    calificaciones.write(
        ...
        str(con_aprobados) + "\n*Suspenso (09-13): " + str(con_suspenso) +
        "\n*Reprobados (01-08): " + str(con_reprobados) +
        "\n\n\n\t\t\t\t\t\t\t------------------------------\n\t\t\t\t\t\t\t\t\t\t\t\tDocente\n\t\t\t\t\t\t\t\t\t\t"
        + docente["nombre"])
...
def funciones_busqueda(opcion3, elemento_buscar):
  if opcion3 == 1:
    ...
    inicio = 0
    fin = len(estudiantes) - 1
    while inicio <= fin and estudiantes[inicio][
        "total"] <= elemento_buscar <= estudiantes[fin]["total"]:
...
while opcion != 4:
  ...
  elif opcion!=4 and len(estudiantes)==0:
